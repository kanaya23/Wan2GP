...
class MultiheadSelfAttentionEnc(nn.Module):
    ...
    def scaled_dot_product_attention(self, query, key, value, attention_mask=None):
        out = self.attn_engine.get_attention()(query, key, value, attention_mask)
    ...
    def forward(self, x, rope, attention_mask=None):
        ...
        q_list = [query]
        query = None
        query = apply_rotary_inplace(q_list, rope)
        k_list = [key]
        key = None
        key = apply_rotary_inplace(k_list, rope)
...
class MultiheadSelfAttentionDec(nn.Module):
    ...
    def attention(self, query, key, value):
        out = self.attn_engine.get_attention()(query, key, value)
        return out.flatten(-2, -1)
    ...
    def forward(self, x, rope, sparse_params=None):
        ...
        if sparse_params is not None and flex_attention is not None:
            try:
